...
def load_all_resources():
    ...
    tok_emb_w = load_hex_w8(os.path.join(MEM, "tok_emb_weight.hex"), DIM)
    ...
    for L in range(N_LAYERS):
        qkv_w = load_hex_w8(os.path.join(MEM, f"block{L}_attn_qkv_weight.hex"), DIM)
        proj_w = load_hex_w8(os.path.join(MEM, f"block{L}_attn_proj_weight.hex"), DIM)
        ff_up_w = load_hex_w8(os.path.join(MEM, f"block{L}_ff_up_weight.hex"), DIM)
