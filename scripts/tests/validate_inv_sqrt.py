"""
Validate tb_inv_sqrt xsim output against Python rtl model.
...
if __name__ == "__main__":
    ...
    max_abs_delta = 0
    sum_abs_delta = 0
    print(f"{'d':>6s}  {'xsim':>6s}  {'rtl':>6s}  {'ideal':>8s}  {'xsim_f':>8s}  {'delta':>8s}  {'err%':>6s}  {'status'}")
    print("-" * 72)

    for entry in xsim:
        d = entry["d"]
        xsim_val = entry["result"]
        rtl_val = rtl_inv_sqrt(d, lut)
        ...
        print(f"{d:6d}  {xsim_val:6d}  {rtl_val:6d}  {ideal:8.5f}  {xsim_f:8.5f}  {delta:+8d}  {err_pct:6.3f}%  {status}")
    ...
    if errors == 0:
        print(f"PASSED - all {len(xsim)} checks match rtl model")
    else:
