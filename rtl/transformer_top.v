...
module transformer_top (
  ...
  input  wire [127:0] tok_emb_data_i,
  ...
  matvec_fp16_w16 #(.IN_DIM(128), .OUT_DIM(256)) u_head_proj (
    .clk_i        (clk_i),
    .rst_i        (rst_i),
    .start_i      (head_start),
    .scale_i      (tok_emb_scale_i),
    .weight_addr_o(head_w16_addr),
  ...
  // tok_emb bus shared between embedding and head_proj
  always @(*) begin
    case (state)
      S_EMBED:     tok_emb_addr_o = emb_tok_addr;
      S_HEAD_PROJ: tok_emb_addr_o = head_w16_addr;
      default:     tok_emb_addr_o = 11'd0;
    endcase
  end
